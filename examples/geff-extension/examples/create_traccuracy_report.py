...
from pixel_patrol_base.api import (
    ...
    process_images,
    get_images_df,
    export_project, add_paths, show_report,
)
...
if __name__ == "__main__":
    ...
    base_path = Path(__file__).resolve().parent.parent.parent / "data" / "geff" / "morgan"
    ...
    exported_project_path = output_directory / "traccuracy_exported.zip"

    # Initialize project with the base directory
    project = create_project("Local Test Image Collections", base_path, loader="traccuracy")

    project = add_paths(project, ["group1", "group2", "group3"])
    ...
    settings = Settings(
        ...
        cmap="viridis",
        n_example_files=5,
        pixel_patrol_flavor=".. but for traccuracy!"
    )
