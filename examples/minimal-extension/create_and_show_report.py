...
The "Pixel Sky Watch" dataset is a handful of tiny `.parquet` tables, each
holding a 16x16 grid of `uint8` values plus fake "image metadata" (time_of_day,
cloud_cover) — read by SkyPatchLoader as if they were photos of a patch of sky.

Steps
-----
1. Generate the tiny dataset (skipped if it already exists).
2. Process the parquet "sky patches" with the custom loader and processor.
   The processor adds ``star_count`` and other columns to the report.
...
if __name__ == "__main__":
    ...
    project = add_paths(project, ["rooftop_log", "campsite_log"])
