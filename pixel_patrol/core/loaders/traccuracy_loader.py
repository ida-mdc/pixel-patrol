...
class TraccuracyLoader(PixelPatrolLoader):
    """
    A precise PixelPatrolLoader for parsing traccuracy JSON result files.

    This loader correctly handles files where each metric block in the 'traccuracy' list
    is a self-contained evaluation. All extracted data (version, matcher, results, etc.)
    is prefixed with its specific metric name, creating a fully dynamic and accurate
    metadata record.
    """
