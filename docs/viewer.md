...
| `violin-quality` | Image Quality Metrics | Violin and box plots comparing image quality metrics (sharpness, noise, contrast/texture, exposure clipping, compression artifacts) across groups. Supports the same **Slice by** toggles and box-summary fallback as Pixel Value Statistics; each plot shows a short explanation and, where the reading is well-established, a directional badge (▲/▼) for which way is better. |
