...
| `image-table` | Image Table | Sortable, searchable table of full-image statistics - one row per image, no per-slice or per-channel rows. |
...
| `metadata` | Metadata | Distribution of pixel data types (`dtype`) and dimension orderings (`dim_order`) per group. Also lists properties shared by all images and available dimension ranges. Requires loader metadata. |
...
| `mosaic` | Image Mosaic | Thumbnail grid, one thumbnail per image. Sortable by any metric (e.g. `mean_intensity`, `laplacian_variance`) to surface visual outliers. Border colors indicate group membership. |
...
| `violin-quality` | Image Quality Metrics | Violin and box plots comparing image quality metrics (sharpness, spectral characteristics, and clipping fractions) across groups. Supports the same **Slice by** toggles and box-summary fallback as Pixel Value Statistics; each plot shows a short explanation and, where the reading is well-established, a directional badge (▲/▼) for which way is better. |
...
| `stats-across-dims-quality` | Quality Metrics Across Dimensions | How image quality metrics change across dimension slices. Useful for detecting focus drift over time (T), channel-specific artifacts (C), or depth-dependent quality changes (Z). A dashed line shows the percentage of images that still have a slice at each position, for spotting datasets where images don't all have the same number of slices. |
