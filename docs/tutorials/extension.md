...
<tr><td><code>read_header(path)</code></td><td><code>(Path) -> FileInfo</code></td><td>yes</td><td>cheap shape/dtype/dim-order probe, <strong>no pixel data loaded</strong>. Optionally set <code>deferred_dims</code> on the returned <code>FileInfo</code> to hint dims that should be split last in memory chunking (e.g. packed colour channels)</td></tr>
...
<div class="wc-flag wc-flag-blue"><span class="fi">💡</span><div><code>dim_order</code> is the only metadata field that must be correct. <code>shape</code>/<code>dtype</code> are always derived from the pixel array, not from <code>meta</code>. The pipeline also overwrites <code>ndim</code>, <code>num_pixels</code>, <code>size_&lt;axis&gt;</code>, <code>dim_&lt;axis&gt;</code>, and the filesystem columns (<code>path</code>, <code>name</code>, <code>type</code>, <code>parent&lt;N&gt;</code>, <code>depth</code>, <code>size_bytes</code>, <code>file_extension</code>, <code>modification_date</code>, <code>imported_path</code>, <code>common_base</code>, <code>child_id</code>) - don't bother setting these. <code>OUTPUT_SCHEMA</code> itself is documentation only: any other field in <code>meta</code> reaches the table whether or not it's declared there, so a loader can pass through metadata it doesn't fully understand.</div></div>
<div class="wc-flag wc-flag-blue"><span class="fi">💡</span><div>To signal that a <code>C</code> dim holds RGB data, set <code>channel_names</code> in <code>meta</code> to a list of names like <code>["Red", "Green", "Blue"]</code> — <code>record_from()</code> picks this up and adds an <code>rgb:C</code> capability automatically, which processors (e.g. thumbnail) use to process the channels as RGB.</div></div>
</div>
