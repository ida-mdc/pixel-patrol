...
byNode['agg']  = {kind:'producer', label:'Aggregation', sub:'adds per-dimension rows',
  desc:'Pixel Patrol offers per-dimension slice statistics in the table. Aggregation of those statistics creates multiple rows per image - one per dimension slice (Z, T, C, ...) - and adds obs_level and dim_* coordinate columns to identify each slice.'};
