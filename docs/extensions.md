...
The pipeline overwrites these after loading, so don't bother setting them: `shape`, `ndim`, `num_pixels`, `size_<axis>`, `dim_<axis>`, `dtype`, and the filesystem columns (`path`, `name`, `type`, `parent`, `parent<N>`, `depth`, `size_bytes`, `file_extension`, `modification_date`, `imported_path`, `common_base`, `child_id`).
