...
- **File and folder paths.** The `name`, `path`, `parent` and `parent<N>` columns store your file and folder names (relative to the base directory), plus each file's modification date. If your files or folders are named after patients, samples, or people, that information is in the table.
