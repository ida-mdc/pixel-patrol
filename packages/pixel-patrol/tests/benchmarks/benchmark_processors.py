# Full standalone copy of the benchmark runner, adjusted to use package-local defaults.
...
def compare_branches(repo_root: Path, branches: List[str], bench_args: List[str], out_dir: Path, snakeviz: bool = False, open_snakeviz: bool = False, allow_clone: bool = False, host_base_path: Optional[Path] = None, use_host_script: bool = True, use_host_data: bool = True) -> None:
    ...
    for branch in branches:
        ...
        try:
            ...
            candidate_paths = [
                Path(wt) / "packages" / "pixel-patrol" / "tests" / "benchmarks" / "benchmark_processors.py",
                Path(wt) / "scripts" / "bench" / "benchmark_processors.py",
            ]
            script_path = None
            ...
            if script_path is None:
                ...
                logger.info("Using branch-local benchmark script for branch %s: %s", branch, script_path)
...
def parse_cli():
    ...
    p.add_argument("--base-path", type=Path, default=Path(__file__).resolve().parent / "benchmark_dataset", help="Base path to dataset (default: package-local benchmark_dataset)")
    p.add_argument("--loader", type=str, default="bioio", help="Loader to use (default: bioio)")
    p.add_argument("--out-dir", type=Path, default=Path(__file__).resolve().parent / "output", help="Directory for branch comparison outputs (default: package-local output")
