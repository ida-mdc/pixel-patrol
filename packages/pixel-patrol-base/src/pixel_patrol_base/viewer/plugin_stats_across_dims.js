...
async function acrossDimsCondensedSummary(ctx) {
  try {
    // The widget plots trends along every dimension that varies, including the
    // spatial X/Y strips - name them all, not just the non-spatial ones.
...
async function acrossDimsCondensedPlot(ctx, container, filterMetric, metricPref) {
  ...
  for (const letter of letters) {
    // Same subset selection as the full widget, for one non-spatial dim.
    ...
    // Mirror the full widget: a mean line with a ±std distribution band per group.
    const traces = [];
    for (const g of ctx.groups) {
