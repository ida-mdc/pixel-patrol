import { q, sample, groupExpr } from '../sql.js';
import { groupColor } from '../colors.js';
import { LEGEND, appendPlot, escapeHtml } from '../plot-utils.js';
...
/**
 * Pixel Value Histograms — matches Dash DatasetHistogramWidget:
 *   - Mode toggle: "Shape" (fixed 0-255 x-axis) vs "Native" (actual pixel range)
 *   - Optional group sub-selection dropdown
 *   - Optional single-file overlay (black semi-transparent bars)
 *   - Averaged per-group fill + line traces
 */
export default {
  id: 'histogram',
  ...
  async render(container, ctx) {
    const hasRange  = ctx.schema.allCols.includes('histogram_min') &&
                      ctx.schema.allCols.includes('histogram_max');
    ...
    controlsDiv.innerHTML += `
      <div style="max-width:360px;flex:1 1 240px">
        ...
        <select id="${GROUP_SEL_ID}" class="form-select form-select-sm" multiple
                style="height:80px">${groupOpts}</select>
        <small class="text-muted">Hold Ctrl/Cmd to multi-select. Empty = all groups.</small>
      </div>
    `;

    // File overlay dropdown (only when name column exists)
    if (hasNames) {
      const nameRows = await ctx.queryRows(`
        SELECT DISTINCT "name" FROM pp_data ${ctx.where}
        ORDER BY 1
        LIMIT ${MAX_FILE_OPTIONS}
      `);
      const nameOpts = nameRows.map(r =>
        `<option value="${escapeHtml(String(r.name))}">${escapeHtml(String(r.name))}</option>`
      ).join('');
      const limitNote = nameRows.length === MAX_FILE_OPTIONS
        ? `<small class="text-muted">Showing first ${MAX_FILE_OPTIONS} files.</small>`
        : '';
      controlsDiv.innerHTML += `
    ...
    // ── Render function (called on every control change) ──────────────────
    const render = async () => {
      ...
      const selectedGroups = groupSelEl
        ? [...groupSelEl.selectedOptions].map(o => o.value).filter(Boolean)
        : [];
    ...
    // Wire controls
    container.querySelectorAll(`input[name="${MODE_ID}"]`).forEach(el =>
...
async function renderHistogram(container, ctx, { mode, selectedGroups, selectedFile, hasRange, hasNames }) {
  const gcExpr  = groupExpr(ctx.state);
  ...
  for (const row of arrowRows) {
    ...
    if (!groupData[group]) {
      groupData[group] = {
        sums:  new Float64Array(counts.length),
        count: 0,
        min:   hasRange ? Number(row.histogram_min) : 0,
        max:   hasRange ? Number(row.histogram_max) : 255,
      };
    }
    ...
    gd.count++;
    if (hasRange) {
      gd.min = Math.min(gd.min, Number(row.histogram_min));
      gd.max = Math.max(gd.max, Number(row.histogram_max));
    }
  }

  const visibleGroups = (selectedGroups.length ? selectedGroups : ctx.groups)
  ...
  const traces = visibleGroups.map(g => {
    ...
    const color  = groupColor(ctx.colorMap, g);
  ...
  // Optional single-file overlay.
  if (selectedFile && hasNames && hasRange) {
    const fileResult = await ctx.query(`
      SELECT "histogram_counts", "histogram_min", "histogram_max"
      FROM pp_data
      WHERE "name" = '${selectedFile.replace(/'/g, "''")}'
      LIMIT 1
    `);
    ...
    if (fileRows.length) {
      ...
      if (counts?.length) {
        ...
        const minV   = Number(row.histogram_min);
        const maxV   = Number(row.histogram_max);
        ...
        traces.unshift({
          type:    'bar',
          name:    `File: ${selectedFile.split('/').pop()}`,
          x:       xs,
          y:       ys,
          width:   Array(nBins).fill(width),
          marker:  { color: 'black' },
          opacity: 0.3,
        });
      }
...
/**
 * Extract a numeric array from whatever Arrow JS v17 gives us for a blob column.
 *
 * histogram_counts is stored as List<Int64> in the parquet (Polars List(Int64)).
 * Arrow JS v17 returns an Arrow Vector for list fields; .toArray() gives a
 * BigInt64Array, so we must convert each BigInt to a regular Number.
 */
/** X-axis bin centers: 'native' maps to [min, max], 'shape' maps to [0, 255]. */
...
function extractBinary(val) {
  if (!val) return null;
  // Native typed arrays — return directly
  ...
  // Plain JS array (e.g. from queryRows / toJSON)
  if (Array.isArray(val)) return val;
  // Arrow Vector (List<Int64> from apache-arrow v17):
  // .toArray() returns BigInt64Array — convert BigInts to Numbers.
