...
import { LEGEND, appendPlot, bargap } from '../plot-utils.js';

const SIZE_NUM_BINS     = 20;
const SIZE_LOG_THRESHOLD = 30;
const MAX_DAYS          = 20;

// file-stats uses wider right/bottom margins to accommodate rotated date labels.
const FILE_STATS_MARGIN = { l: 50, r: 80, t: 50, b: 80 };

/**
 * File Statistics — matches Dash FileStatisticsWidget:
 *   - File count + size by extension (grouped/stacked bars)
 *   - File count by size bin (log-spaced if range is large)
 *   - File count by modification date (day / month)
 *   - Invariant properties table for constant values
 */
export default {
  id: 'file-stats',
  info: [
  ...
  async render(container, ctx) {
    ...
    if (exts.length === 0) {
      // No extension data — nothing to show
      ...
      renderGroupedBars(container, {
        ...
        groups,
        getValue: (ext, g) => {
          const r = extRows.find(r => String(r.ext) === ext && String(r.__group__) === g);
      ...
      renderGroupedBars(container, {
        ...
        getValue: (ext, g) => {
          const r = extRows.find(r => String(r.ext) === ext && String(r.__group__) === g);
          return r ? Number(r.total_bytes ?? 0) : 0;
        },
        colorMap: ctx.colorMap,
    ...
    if (nUniq <= 1) {
      ...
      if (breaks.length) {
        ...
        const xLabel = useLog ? 'File size (log-spaced bins)' : 'File size bin';
        renderGroupedBars(container, {
          categories: labels,
          groups,
          getValue: (bin, g) => {
            const r = sizeBinRows.find(r => r.bin === bin && String(r.__group__) === g);
            return r ? Number(r.count) : 0;
          },
          colorMap: ctx.colorMap,
    ...
    if (dateRows.length) {
      ...
      if (uniqueDays.length === 1) {
        ...
        // If too many days, roll up to months.
        let displayRows = dateRows;
        let dateLabel   = 'Date';

        if (uniqueDays.length > MAX_DAYS) {
          // Re-query at month granularity.
        ...
        renderGroupedBars(container, {
          ...
          groups,
          getValue: (d, g) => {
            const r = displayRows.find(r => String(r.day) === d && String(r.__group__) === g);
            return r ? Number(r.count) : 0;
          },
          colorMap: ctx.colorMap,
...
function renderGroupedBars(container, { categories, groups, getValue, colorMap, title, xLabel, yLabel, showLegend = false }) {
  const traces = groups.map(g => ({
    type:   'bar',
    name:   g,
    x:      categories,
    y:      categories.map(cat => getValue(cat, g)),
    marker: { color: groupColor(colorMap, g) },
  }));
  ...
  appendPlot(container, traces, {
    ...
    title:      { text: title },
    barmode:    'stack',
    bargap:     bargap(categories.length),
    xaxis:      { title: xLabel, type: 'category' },
    yaxis:      { title: yLabel },
    height:     400,
    showlegend: showLegend && groups.length > 1,
    ...(showLegend && groups.length > 1 ? { legend: LEGEND } : {}),
  }, 'margin-bottom:24px');
}
...
function computeSizeBins(minS, maxS, nUniq) {
  ...
  for (let i = 0; i < edges.length - 1; i++) {
    labels.push(`${formatBytes(edges[i])}–${formatBytes(edges[i + 1])}`);
  }
...
function buildSizeCaseSQL(breaks, labels) {
  ...
  for (let i = 0; i < breaks.length; i++) {
    sql += ` WHEN "size_bytes" < ${breaks[i]} THEN '${labels[i]}'`;
  }
