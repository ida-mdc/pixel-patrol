...
/**
 * Critical rendering order: all tables must be in the live DOM before any
...
async function renderAcrossDims(container, ctx, filterMetric) {
  ...
  const dimLetters = [...new Set(
    Object.values(byMetric).flat().map(g => g.letter)
  )].sort();
...
function makeAcrossDimsPlugin(id, label, info, filterMetric) {
  return {
    ...
    info,
    requires(schema) {
      return detectDimMetricGroups(schema.dimMetricCols ?? [])
        .some(g => filterMetric(g.base));
    },
...
export default [
  makeAcrossDimsPlugin(
    ...
    'Basic Statistics Across Dimensions',
    BASIC_INFO,
