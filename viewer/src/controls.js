...
const EXPORT_HINTS = {
  'csv:summary':     'One row per image — compatible with Excel, pandas, R.',
  'csv:full':        'Full table - including rows of dim slice statistics.',
  'parquet:summary': 'One row per image in Parquet format.',
  'parquet:full':    'Full long-format data: one row per observation level and dimension slice.',
};
...
function buildExportControls(schema, onExport, canParquet) {
  ...
  function updateHint() {
    hintEl.textContent = hasSlicing ? (EXPORT_HINTS[selectEl.value] ?? '') : '';
  }
