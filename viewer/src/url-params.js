 ...
 *   learn     - "0" when condensed-mode intro boxes are disabled (on by default)
...
export function writeUrlParams(state) {
  ...
  setOrDelete(params, 'learn',  state.condensedMode ? null : '0');
  ...
  setOrDelete(params, 'opened', state.openedWidgets?.size > 0
    ? [...state.openedWidgets].sort().join('.') : null);
...
export function readUrlParams() {
  ...
  if (params.has('learn'))  out.condensedMode      = params.get('learn') === '1';
